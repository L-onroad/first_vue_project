...
export default {
  ...
  mounted() {
    this.$store.dispatch("getGoodsInfo", this.$route.params.goodId);
  },
  computed: {
  ...
  methods: {
    // 改变点击商品属性的样式
    changeChecked(spuSaleAttrValue, arr) {
      arr.forEach(item => {
    ...
    async addShoppingCart() {
      try {
        await this.$store.dispatch('addOrUpdateGoods', {goodId: this.$route.params.goodId, goodsNum: this.goodsNum})
        this.$router.push({name:'addCartSuccess', query: {goodsNum: this.goodsNum}})
        sessionStorage.setItem('ADDCARTSUCCESS', JSON.stringify(this.skuInfo))
      } catch (error) {
        alert(error.message)
      }
    }
